...
namespace VSEmbed.Services {
	class FakeDTE : DTE {
		public Commands Commands { get { return new FakeCommands(); } }
		class FakeCommands : Commands {
			// Used by Microsoft.Internal.VisualStudio.Shell.KeyBindingHelper
			public Command Item(object index, int ID = -1) {
				return null;
			}
			...
			public Command AddNamedCommand(AddIn AddInInstance, string Name, string ButtonText, string Tooltip, bool MSOButton, int Bitmap, ref object[] ContextUIGUIDs, int vsCommandDisabledFlagsValue = 16) {
				throw new NotImplementedException();
			}
